...
class TextEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.bert = BertModel.from_pretrained('bert-base-uncased') # load pre-trained BERT or download it if not available

# Freezing so it doesn't train
        for param in self.bert.parameters():  
            param.requires_grad = False


#Compression layer to reduce BERT output to 128-dim
...
# Forward Pass 
    def forward(self, input_ids, attention_mask):
        ...
        # Use [CLS] token representation  (CLS token is used to capture meaning of the entire sentence which becomes pooler_output which is a summary of the input sequence)
        pooler_output = outputs.pooler_output

        return self.projection(pooler_output)
# End of TextEncoder

#Video Encoder using 3D ResNet-18

class VideoEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = vision_models.video.r3d_18(pretrained=True) # Getting pre-trained 3D ResNet-18 layer model for video feature extraction
        
        # No training for backbone (make it frozen so it is faster and requires less memory) 
        for param in self.backbone.parameters():
            param.requires_grad = False

        # Modify the final fully connected layer to output 128-dim features
        num_fts = self.backbone.fc.in_features
        self.backbone.fc = nn.Sequential( # Creating Linear Layer 
...
class AudioEncoder(nn.Module):
    def __init__(self):
        ...
        # Freezing conv layers no training
        for param in self.conv_layers.parameters():
            param.requires_grad = False

        #  Projection layer to 128-dim for training 
    ...
    def forward(self, x):
        # Input x shape: [batch_size, time_steps, feature]
        x = x.squeeze(1) # Removes the dim to make a 2D tensor [batch_size, feature, time_steps]
...
class MultimodalSentimentModel(nn.Module):
    def __init__(self):
        ...
        self.fusion_layer = nn.Sequential(
            nn.Linear(128 * 3, 256), # Combining 3 modalities each of 128-dim and outputting 256-dim
    ...
    def forward(self, text_inputs, video_frames, audio_features):
        ...
        combined_features = torch.cat([
            ...
            video_features,
            audio_features
        ], dim=1)  # [batch_size, 128 * 3]  # if dim=0 then it will stack the batches and not features 
